...
gulp.task('build.dev', function (done) {
    return runSequence('clean.dist', 'tslint', 'build.jslib.dev', 'build.sass.dev', 'build.js.dev', 'build.csslib.dev', 'build.fonts', 'build.index.dev', done);
});
...
gulp.task('serve', function (done) {
    return runSequence("build." + config_1.ENV, 'server.start', 'watch.serve', done);
});
gulp.task('radic', function () {
    console.log('yes');
});
// --------------
// Build prod.
